...
####web development
Django is popular python framework(a big libraby is called a framework) that simplifies build websites like amazon or flipkart. 
...
If you are familar with C or java, you would noticed the is not variable declaration like `int age = 32;`. In python, you don't declare variables before assigning some infomation. you just use them. Also, the semicolon\(`;`\) is optional in python-you can use it but is not required. 
...
lets write a program to print numbers from 0 to 4.
```{python}
print 0
print 1
print 2
...
print 4
```
now, lets write a program to print numbers from 1 to 10
```{python}
print 0
print 1
print 2
...
print 9
```
that is very repeatative and tedious. i can't write code like this. what if we need print a 100 numbers? 
The solution to this is loops. they are types of them - for loop and while loop.
...
for number in numbers:
  print number
```
the for loop executes the print a number of times. each time the variable number is assigned a value from numbers. An equivalent non-for code would be
```{python}
numbers = range(0,5)
number = numbers[0]
print number
...
print number
```
you write loops not just with numbers but also with strings.
...
while condition:
  ...
  stmt
```
As with if block and for block, while block also uses the `:` and indentation. The statements within the while block will repeatedly executed till the condition evaluates to False.
