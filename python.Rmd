...
```
`raw_input()` always takes the input as string. Even if we type numbers, they would be taken as string. So, if we really want to input numbers, we need to convert them. We use the built-in python functions `int()` and `float()` for this.
```{python, eval=FALSE}
a = raw_input("enter a number:")
print a
print type(a)
...
print queue.deque()
```
Notice in the output, the message we coded was printed. We can also raise a more specific exception(like `IndexError`) instead of the generic superclass `Exception`. Or we could just create our own exception class. Let's create a exception called EmptyQueueError.
...
##Unit Testing in Python: Writing and running Tests
