...
Installation steps:
Ipthon requires visual c++. It can be downloaded at `http://aka.ms/vcpython27`. Download and install this software. It is pretty easy and shouldn't cause any troubles.
...
```
Python seperates blocks based on their indentation. Line 3,4 are within the if block and will only execute if the condition is True. Notice unlike other langauges, python doesn't use curly brackets{} for creating blocks.
```{#numCode .python .numberLines}
a,b = 10,20
if a > b:
...
## Scope
Variables declared(appear for the first time in code) in the function(known as local variables) are not accessable outside the function.
```{python, error=TRUE}
def some_function():
...
print variable
```
Vaariables declared outside the function(known as global variables) are available inside a function but can't be modified directly.
```{python}
...
function()
```
If you try to modify, python will think that you are creating a new **local variable** that coincidentally has the same name as the global varaible.
```{python}
variable = 'global_variable'
def function():
...
Python comes a rich libraby for doing pretty much anything under the sun. These libraries are available in the form of modules. Some of the modules are avaiable with python, but some are not. We can simpley import a module to check if a module is present or not. 
```{python, error=TRUE}
import flask
```
```{python, echo=FALSE}
...
As you see, the python interpreter says flask doesn't exist. Let's install it. Open windows command line by pressing the windows key + r. Or search for an app called `run` from the start button. Type 'cmd' and press enter. you are in the windows command prompt. Now install the module 'flask'
...
If you get an error with pip command, it be that the path is not set. In this case, you could go to the folder that python was installed in. It normally is `C:\Python27\`. check your python folder is this. Now, navigate to this folder in the windows command prompt. 
```{bash, eval=FALSE}
...
Classes are neat way to organize code. Consider we write code for a box. A box has 3 properties-height, width, bredth. We can use variable like box_height, box_width, box_bredth to represent them. But doing this makes 3 seperate variables that seem to have no relation. Instead, we could create a class that holds these variables.
```{#numCode .python .numberLines}
class Box(object):
...
print box.volume()
print box.height, box.weight, box.bredth
another_box = Box(3,2,1)
print another_box.height, another_box.weight, another_box.bredth
print another_box.volume()
```
I know that's a lot of code. Don't worry. It's not hard. Use of classes consists of 3 parts- defining a class, creating objects and calling methods on them.

**Class Defination**

We are creating a class called Box that contains variables height, weight and breadth. So, think of class as a container to holding varaibles. Remember, objects and classes are related but are different.Class is a blueprint-a plan that can be used to create objects. line 1 to 15 is the class defination whereas line 18 and line 21 is the object creation. Only one class can exist but several objects can exist. On line 18 and line 21, are creating 2 objects. These 2 are different from each other. Each object have a set of height, weight, bredth are have no connection from another. 

**Object creation**

line 17 creates a box object with height as 2, weight as 3 and bredth as 4. At this line, the special function `__init__()` is called with 2,3,4 as parameters. I know, there is no call to `__init__()`. Here is the thing with python: If a class is defined with name `XYZ` then we create an object by calling `XYZ()`. However, inside the class there is no such method. Instead the class `XYZ`'s `__init()__` is called. Notice the print stmt 'creating object' executed each time an object is created.

Notice that the contructor defination on line 2 has 4 parameters while, called the method call on line 17 we had only 3 parameters. This is because all class methods have a default first parameter as `self` which doesn't have be passed when the method is called. you can notice this with the mehtod `volume()` as well. the defination on line 13 has one parameter but on line 18, while calling there is none.

`self` is special variable that refers to the object itself. All the object variables are refered with the self variable. line 3 to line 5, we define the 3 instance variables. We are also assigning them h,w,b values. A Class can also has functions. We are defining the method `volume()` on line 13,14 within the class.

line 2 to 5 is function defination named `__init__()`. `__init()__` is specail method called the constructor. It is called each time an object is created. All the variables inside an class should be initialited within the constructor. Our Box class has 3 varaibles and there are assigned inside the constructor. unlike java, we can't declare instance variables in python and hence they need to be initiated within the constructor.

**Accessing instance variables**

we can access instance varaibles either from within the class or outside the class. Within class, we use the self varaible to accessing(by saying `self.variable_name`). on line 14, all 3 variables are accessing.
When we are referring outside the class, we use the varaible that pointes to the obj. like we on line 20 as `box.height`. 
...
print family
```
What we have achivied here is: Family is type we build just now. The python language didn't know about this before now. len is built-in function. we defined our class to work seemlessly with python's built-in functions. `__len__(self)` is a magic method that gets called when `len(family)` is encountered. 
```{python}
class Family(object):
  def __init__(self, mems):
    self.members = mems
...
doctor.heal(cat)
```
Yes, he can. A Cat is a Animal. And Doctor heals Animal. And hence Doctor can treat a Cat. This is power of Polymorpish. The Cat code didn't even exist when the Doctor code was written. The Cat code was written after the Doctor code. Even through, Doctor code worked seemlessly with the Cat code.
```{python, echo=FALSE}
class Animal(object):
  def __init__(self):
    self.bp = 100
...
Notice instead of object, we are subclassing Exception instead of teh usaul object. This is because we the functionality of the Exception class.
...
```
Explanation:
`open()` is a standard python function for opening files. The first parameter is the name the file. The second is the mode. Modes can be 'r' for read, 'w' for write and 'a' for append. For 
...
```
Explanation:
`self` is a variable every object has that points to itself. In the above code, we are checking `obj == self` where a is passed as obj. Observe that it results in True. 
i) Robot
```{python}
class Robot(object):
    def __init__(self):
