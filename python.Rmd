...
This is a *tiny, tiny* book. It was meant to be tiny. It's about 50 pages. Without lab exercises, it's 34 pages. Why? They say, a picture is worth a thousand words. Similarly, an example is worth a page of explanation.
...
```
The `if` block is executed only if the condition is `True`. In the above code, line 3,4 are executed only if condition `a > b` is `True`. One key point: **Python seperates blocks based on their indentation.** Line 3,4 are within the `if` block. Unlike other langauges, python doesn't use curly brackets{} for creating blocks. More examples:
```{python}
a,b = 3,2
if a < b:
