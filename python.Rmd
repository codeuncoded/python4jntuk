...
This is a *tiny, tiny* book. It was meant to be tiny. It's 50 pages. Without lab exercises, it's 33 pages. Why? They say, a picture is worth a thousand words. Similarly, an example is worth a page of explanation.
...
If, at times, you feel the explanation is not enough, be patient and complete the examples. I assure you as you go along, things will be clearer. Although the lab examples are in a seperate section, they form a good repertoire of examples. So don't skip them. 
...
Data science is different from the general programming. Generaly, finding out the average of subjects marks is a typical programming problem. Data science trys to predict the answer to question 'When will you come to college tommorow?' It does it by observing previous data. Since you came at 8:30 everyday for the past 6 months, you will come tommorow at 8:30. This is a simple problem but typical problems in this field are like 'what is chance of getting cancer if you have a family history?' 'what will the weather be like today?' etc. Two langauges dominate this field - Python and R. Both of which are in our syllabus.

####Gaming and GUI applications
The famous game Civilization IV, where you build an empire, was written in Python.

####Internet of Things(IoT)
Our calculators are computers. Not as powerful as our laptops but still are computers. Iot is a technology where, small calculator sized but more powerful computers are attached to everyday devices like TV, Fridge, AC to make them smart. The smart house-where you can voice operate lights, doors, music, etc-is because of iot. And, Python is the primary language of this field.
...
Once you download the exe file, just double click and install. 

Press next to keep the default folder as 'C:\\Python27\\'. On the 'Customize Python 2.7.13', select the feature, 'Add python.exe to Path'. Make sure that this feature is selected (no cross mark adjacent to this feature). 

Keep pressing next till the installation is complete. This should be very easily.

<!-- ![Install Python](./python download_LI.jpg) -->
Congrats! You now have Python installed on your machine.

### Using the Python interpreter
Click on windows start and find Python. In windows 7 and above, you could just type in Python.

Click on **python(command line)**. A black screen will appear. This is called the **python interpreter**.

This is where we will run all our Python code. We will open the python interpreter each time we need to do some coding, so become comfortable opening it.

### (Optional IPython installation)
All Python code in this book can be done on the python interpreter we just installed. However, IPython is a better tool than the Python interpreter. I highly recommend that you install IPython as well. If you do install it, use IPython whenever I say Python interpreter.
...
ipython
```
Remember, you can use either the Python interpreter or IPython.


##1.3 REPL
Open the **python interpreter**(if you installed IPython, open that instead of this) and type in the following, one line at a time:
```{python, eval=FALSE}
2 + 3
45 + 21.1
45/9
...
Save the previous code listing into a file and name it script.py and run that file using the command:
```{python, eval=FALSE}
run script.py
```
...
name = 'sreekanth'
```
Here, `age` is variable to which the infomation 32 is saved. `name` is another variable to which the infomation `'sreekanth'` is saved. We can see what infomation a particular variable has simply by printing them(don't worry about the quotes, more on that later).
```{python, eval=FALSE}
print age
print name
```
...
print name
```
`print` is the built-in Python command to print variables or values. We could also modify the infomation assigned to a variable.
```{python, eval=FALSE}
age = age + 10
print age
name = name + ' kolamala'
...
If you are familar with C or java, you would noticed that there is no variable declaration like `int age = 32;`. In Python, you don't need to declare variables before assigning some infomation. You just use them. Also, the semicolon `;` is optional in Python. 
...
##2.1 Types
Based on what operations we can do on them, variables are grouped into so called **Data types**.
Python has 5 data types - `Numbers, Strings, Lists, Tuple, Dictionary`. In this unit, we will discuss `Numbers` and `Strings`. We will see the other 3 in the next unit.
...
a = 12
print type(a)
#type is a built-in function we can use to check a variable's type
b = 12.3
print type(b)
...
###Strings
Strings are used for saving text like `'India'` or `'Cake'`.Strings can be surrounded with either single quotes or double quotes. Unlike in C, there is no difference between char and string-both are same.
```{python}
name = "sreekanth"
...
```
###Booleans
`int` variables can be assigned any whole number. `string` variables can be assigned any charecters. `bool` variables(otherwise known as booleans) are special. They can be assigned only 2 values - `True` or `False`
```{python}
flag = True
print flag
flag = False
...
```
`raw_input()` always takes the input as string. Even if we type numbers, they would be taken as string. So, if we want to input numbers, we need to convert them. We use built-in functions `int()` and `float()` for this.
```{python, eval=FALSE}
a = raw_input("enter a number:")
print a
print type(a)
...
print type(a)
```
Also note that the type of a variable can change during program executation.
...
a, b = 10,20  #a is assigned 10 and b is assigned 20.
print a,b
a,b = b,a
...
Arithmetic operators are used for doing common arithmetic operations like addition, substraction etc. They are `+ - * / % **`. For example, `2/3` is `2` divided by `3`.

`a%b` gives the reminder when `a` is divided by `b`. `a**b` gives `a` to power of `b`. Few examples for clarity are given in the table below.
...
if condition:
  ...
  stmt
```
Line 2 to 4 is the `if` block. The `if` block is executed only if the condition is `True`. One key point: **Python seperates blocks based on their indentation.** Line 2,3,4 are within the `if` block. Unlike other langauges, python doesn't use curly brackets{} for creating blocks. More examples:
```{python, eval=FALSE}
...
```
We can also use a `if-else` block. The `else` block is executed if the `if` condition fails.
```{python}
a,b = 3,2
if a < b:
...
```
Here, the condition `a<b` results in `False`.The `if` block is skipped and the `else` block is executed.
```{#numCode .python .numberLines}
a,b = 10,20
if a > b:
...
if a > b:
  ...
  pass
```
`pass` is a keyword that represents an empty statement. It's is like an empty ; in C. Again, observe the indentation. Line 3 is inside the if block and line 5 is inside the else block. We can check more than 1 condition using the `if-elif-else` block.
...
if condition_1:
  ...
  stmts_3
```
The following table gives which statements are executed based on the values of if conditions.
...
print numbers
```
Notice how the list is re-adjusted. All the elements after index 1 are moved one place left to fill the list. The length of list is also reduced by 1.
```{python}
numbers = [10,20,30,40,50]  #creating a list
...
print sublist
print numbers[0:3]
print numbers[:3] #missing starting number will be replaced with 0
print numbers[4:7]
...
#we can concatinate(combine) two strings too
fruit = name + "apple"
print fruit
```
...
print 4
```
Now, let's write a program to print numbers from 1 to 9.
```{python}
print 0
print 1
print 2
...
print 9
```
This is very repeatative and tedious. And what if we need print 100 numbers? We can't write code like this.
...
for number in numbers:
  print number  #Notice the indentation. Only this line is within the for loop
```
...
print "program end"
```
Notice that this program prints `0` and exits the `for` loop. When `i=1`, the `if` condition is `True`, and `break` statement is hit; When a `break` statement is hit; the loop, the `break` statement is in, is immediately exited.
...
Built-in functions exist for common mathematical set operations like union, intersection.
```{python}
setA = {1,2,3,4,5}
setB = {4,5,6,7,8}
...
##3.5 Dictionaries
Dictionaries(commmonly shortened to dict) are *{key,value}* pair data structures. Each element in a dict is a pair of key and value. Keys have to be uquine. Keys can be thought as indices with names. Dicts are similar to Java hashmaps. Let's create a dict and play around.
```{python}
wordFrequency = {'a':1, 'b':5, 'c':10, 'e':15}
#Here all the strings('a', 'b', 'c', 'e') are keys
#all the numbers(1,5,10,15) are values
#each key has a associated value.
#wordFrequency can be used to store frequency of alphabets in a string

#key can be used access value. Like an index is used in a list
...
wordFrequency['x'] = 4
print wordFrequency
#find out if a key exists in dict using the 'in' keyword
print 'd' in wordFrequency  
```
...
Let's write a function that returns cube of a given number. And find the cubes of a list of numbers.
```{python}
def cube(number):
  return number**3
print map(cube, range(10))
```
Instead of defining a function for such a simple task, we can use an **anonymous function**. Anonymous functions are like regular functions except they don't have a name. They defined using the keyword `lambda`. 
```{python}
...
print sums
```
We can see that we were able to element-wise add from 3 lists without the need to declare a new function. If we had to add 4 lists, we could easily write an anonymous function instead of declaring a new funcition.
...
##4.3 Scope of Variables in a Function
Variables declared(appear for the first time in code) inside a function are known as **local variables**. Local variables are not accessible outside the function.
```{python, error=TRUE}
def some_function():
...
def function():
    ...
    global_variable = 20  #this change will be visible outside the function as well
...
print pi  #can't use anything else within the module.
```
...
print m.sqrt(25)
```
Finally, we can also import code written by us in the same way. An example is given in the unit on testing.

##4.5 Python packages and pip
...
Use of classes consists of 3 parts - defining a class, creating objects of that class and calling methods on them.

**Class Defination**

Line 1 to 15 is the class defination. A class defination tells what variables and functions the class should have. We are creating a class called `Box` that contains variables `height`, `weight` and `breadth`. So, think of class as a container for holding variables.   
A `Class` can also has functions. We defined the method `volume()` on line 14,15 within the class.

**Object creation and method calling**

Line 18 creates a box object with `height` as 2, `weight` as 3 and `breadth` as 4. At this line, the special function `__init__()` is called with `2,3,4` as parameters. I know, there is no call to `__init__()`. Here is the thing with python: If a class is defined with name `XYZ` then we create an object by calling `XYZ()`. However, inside the class there is no such method. Instead the class `XYZ`'s `__init()__` is called. This is special method called **constructor**. Another object is created at line 21. Notice the print stmt 'creating object' executed each time an object is created.

Only one class can exist but several objects can exist. On line 18 and line 21, 2 objects are created. These 2 are different from each other. Each object have a set of `height, weight, breadth` and have no connection from one another.
...
##5.2 Magic methods
The beauty of python is in its magic methods. Let's define a class to represent a family.
```{python}
class Family(object):
...
# We can use the len() function to find out how many family members are there.
...
print family[1]
```
As you can see in the output, when we printed `family`, instead of printing object id(as was the case in the previous example), the contains of the list `members` was printed. This is because of the magic method `__str__()` which is called when the object is printed. In summary, magic methods are defined in classes to work well python's built-in methods.
...
class South_Indian_Family(Family):
  pass
  # pass is a empty statement. It don't do anything
...
Notice that the `South_Indian_Family` had no methods or variables defined. But, we are able to access `southy.members`. This is because `South_Indian_Family` derived the variable `members` from its base class.
...
dog.make_sound()  #the Dog class' make_sound() will be called and not Animal's method.
dog.attack_visitor()
animal.attack_visitor() #error: remember attack_visitor() is not defined in Animal.
```

##5.5 Polymorphism
\indent Lets code up a class `Doctor` that heals the animal when it is sick. To keep this simple, lets assume that the doctor checks for the animal's bp(blood pressure) and the sound that the animal makes.
```{python}
class Doctor(object):
...
doctor.heal(cat)
```
Yes, he can. A `Cat` is a `Animal`. And `Doctor` heals `Animal`. And hence `Doctor` can treat a `Cat`. This is power of polymorpism. __The `Cat` code didn't even exist when the `Doctor` code was written. The `Cat` code was written after the `Doctor` code. Yet, `Doctor` code worked seemlessly with the `Cat` code.__
```{python, echo=FALSE}
class Animal(object):
  def __init__(self):
    self.bp = 100
...
```
Looks good. This is an never ending program (press ctrl+z to stop). Lets see what happens when we give 0 for denominator
...
Had there been no exception, the try block would have completely executed and the except block would have never executed. Remember, we write error handling code in except block. So, if there is no error, there is not need for the error handling code to be executed.
...
`ZeroDivisionError` and `ValueError` are two types of exceptions. Infact there are derived from the class called `Exception`. A lot of exceptions are predefined by the language and all of them are derived from the class `Exception`. So, instead of using these, we can use the superclass `Exception`.
```{python, eval=FALSE}
while True:
	try:
		a = int(raw_input("enter numerator:"))
		...
		print 'Bad input. Please try again'
```
However this is bad practice and should never be done. The reason being: this would mask exceptions which the programmer not intented to. Like the above will run infinetely even if you press ctrl+z as that would also be caught as bad input. Something we as a programmer not intended to handle.
...
print queue.deque()
```
Notice in the output, the message we coded was printed. 

### User Defined Exceptions

We can also raise a more specific exception(like `IndexError`) instead of the generic superclass `Exception`. Or we could just create our own exception class. Let's create a exception called EmptyQueueError.
...
Notice that instead of object, we are subclassing `Exception`. So that we can get the functionaly of Exception class.

\newpage

#Unit 6 Testing

##6.1 Why is testing required?
...
even(4)
```
I purposefully placed an error on line 3 of the `even_or_odd()` function. `num` is number and cannot be directly concatinated with a string. It should be converted into a string first. We did this on line 5. This is the reason that the program did not report error for the `even(3)`, because the program never hit line 3. The if condition failed and line 3 was never executed. Whereas, when `even(4)` was called, `if` condition passed and line 3 was hit. And then, the error came up. 

In staticly typed langauages like C and java, this error would have been a compile time error. In python, it's a runtime error. Hence, we can't be sure that our code is error-free till all the lines in our code are executed. This is where testing comes. No piece of software can be written in python without a thorough testing.

Luckily, Testing in python is easy.

##6.2 Unit Testing in Python: Writing and running Tests

Testing individual functions or classes is called **Unit Testing**. Testing how well two modules work with each other is called **Intregation Testing**. Unit testing is in our syllabus.
Let's say we want to test the function `reverse_string()`. 
```{python}
def reverse_string(sentence):
	return ''.join(reversed(sentence))
```
As a test engineer, *what* this function does is important but not *how* it does. This function reverses a string passed to it. We could have 3 test cases:
...
Let's write the first test case and run it. Python has built-in module unittest - a testing framework. 

We write test code seperate from our actual code. We create a class to hold all our test cases. You can name it everything but it should inherit from the TestCase class. Here, we name it `ReverseStringTestCase`.
...
Each test case is written as a method. `test_reverse_string_random_string()` will test the first test case. Note that our class inherited the class `TestCase`. The cool thing about this class is it has few helper functions like `assertEqual(a,b)` that throws an error if `a` and `b` are not equal. 

In our test case, we call `assertEqual('random', reverse_string('modnar'))`. If `reverse_string()` works as intended, it should return `'random'` which would be equal to the first argument of `assertEqual()`. Else, it throws an error and the test case will fail. 
...
Write the below lines in file named script.py and run that file with the following command.
```{python, eval=FALSE}
print 2 + 3
print 45 - 21.1
print 45/9 + 2*1.2
...
```
b\) Write a program add.py that takes 2 numbers as command line arguments and prints their sum.
```{python, eval=FALSE}
...
print "odd" if number % 2 else "even"
```
There is another version of this program using functions in the section on functions. You might want to have a look.
...
```
**Explanation:** We are using the famous Sieve of Eratosthenes(https://en.wikipedia.org/wiki/Sieve_of_Eratosthenes). It's a algorithm to find primes up to a limit. It does this by iteratively removing multiples of each prime. 

From natural numbers, first multiples of 2 are removed. Then multiples of 3 are removed. Then of 5 and so on.
...
a\) Write a program to count the number of characters in the string and store them in a
...
print dict(collections.Counter(name))
```
**Explanation:** `collections.Counter` is built-in dict class that counts the frequencies of elements. It also has the method `most_common()` that returns the most common element.
...
```{python}
print 'split is a string method that splits a string by a given pattern'.split(' ')
...
I don't understand what 'these lists' mean. So, can't really answer this.
...
fileContent = text_file.readlines()  #returns a list of strings: one string per line in the file
text_file.close()
fileContent = ''.join(fileContent)  #combining all lines to form one single string
print countFrequencies(fileContent)
# countFrequencies() is the function we wrote in exercise 5a. you need to include or import that
```
Output is omitted since it would depend on the contents of file `sample.py`. 

**Explanation:** `open()` is a standard python function for opening files. The first parameter is the name the file. The second is the mode. Modes can be `'r'` for readonly, `'w'` for (over)write and `'a'` for append. `open()` returns a file descriptor. `readlines()` is method of the descriptor that returns all the lines in the file as a list. Every file we open, we need to close it be calling its `close()` method.

Can you use character
frequency to tell whether the given file is a Python program file, C program file or a text file?

C programs need a semicolon on each statement. Python needs none. And a text file might contain few semicolons but not as much as a C file. So, based on the number of semicolons in a file, we can guess if it's a C or Python or text file. Ofcourse this method is not completely foolproof. One can always create a file that contradicts the above premise.
...
text_file.close()
```
Create a file named sample.txt and write some text in it. Output of this program is omitted as it would be depend on the text in that file.
...
```
**Explanation:** `with` is a keyword that lets us create context managers. Context managers are a clearer way to write code. Notice that in this solution, we didn't close the file. That is done by the context manager. Context managers are not in the syllabus. So don't worry if you don't understand them. Just remember that they are a neat way to organize code.
...
def median(numbers):
    #this condition will pass when numbers is empty
    if not numbers:
        return 0
    #sorted() is built-in python function
...
Median is the middle most number when all the numbers are in an ascending order.

The most common number is called the Mode. So, we count the frequencies of all elements and find the element with the highest frequency. 
...
```
**Explanation:** `zip` is built-in function that takes two or more sequences and returns a sequence of tubles that contain the corresponding elements the sequences. This is very useful when we need to iterate over more than one sequence simultaneously. Few examples:
```{python}
...
def dups(array):
    import collections
    #you can also use countFrequencies from ex 5 instead of Counter
...
**Explanation:** `pop(index)` is a list method that removes and returns the element at `index`. `index` has a default value of `length - 1`. That means `pop()`(without `index`) removes and returns the last element in the list.

Greatest Common Diviser(gcd) of two numbers a,b lies between 1 to max(a,b). So, we create a list of all the numbers that are from `1` to `max(a, b)` and that also divide `a,b` prefectly. Since gcd is the maximum of all these numbers, we pop the last element. 

Least Common Multiple(lcm) of two numbers a,b lies between max(a,b) and a*b. So, we create a list of all such numbers and which are also perfectly divided by a,b. Since lcd is the min of all these numbers, we pop the first element.

##Exercise 11 Multi-D Lists
a\) Write a program that defines a matrix and prints it.
```{python}
matA = [[1,2,3],
        [4,5,6],
        [7,8,9]]
...
**Explanation:** We discussed about `map` in the Functions section. `map` takes a function and applies it to a list(or lists) to create a new list. But what if instead of a function, we need to a simple operation like addition. We can't pass the operator `+` to map. For these kind of situations, python has built-in functions that behave exactly like operators. `operator.add()` is one such example. It mimics `+` operator. 
```{python}
import operator
print operator.add(2,3)
```
There are others like `operator.mul()`, `operator.sub()` etc. Even functions that mimic logical operators like `operator.gt()` (for `>`) and `operator.not_()` (for `not`) exist.
...
def multiply_matrix(mat_a, mat_b):
    a = len(mat_a)
    b = len(mat_b[0])
    # creating a default array filled with zeroes
...
print r.status_code
#print r.text
#uncomment the above line for content of the site
```
...
This book is free for your use. You can share it with anyone as along as you don't modify it. Feel free to share it with your friends or colleagues. Do send your feedback to sreekanth.kolamala@gmail.com
