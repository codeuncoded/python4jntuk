...
```
`map` is built-in function that takes a function and *applies* it to a list. Each element in the list is used a parameter for `len()` and the results form a list. Notice that `len` is passed and not `len()`. This is because, we are not calling `len`. We are just passing `len` to map. `map` will internally call it.

Let's write a function that returns square of given number. And find the squares of a list of numbers.
```{python}
def square(number):
  return number**2
...
```{python}
#adding indivial numbers in lists.
sums = map(lambda x,y: x+y, range(0,5), range(5,10))
print sums
```
mental note: add an example of lambda with a return statement.
...
## Scope
Variables declared(appear for the first time in code) inside a function(known as local variables) are not accessable outside the function.
```{python, error=TRUE}
def some_function():
...
## Import statements
Often times, instead of writing code ourselves, we could you code written by others. This code is available in the form of libraries. We need to declare them before using them by writing an import statement. There are 2 ways of doing this:
...
Python comes a rich library for doing pretty much anything. These libraries are available in the form of modules. Some of the modules are available with python, but some are not. We can simply import a module to check if a module is present or not. 
```{python, error=TRUE}
import flask
```
```{python, echo=FALSE}
...
Open windows command line by pressing the windows key + r. Or search for an app called `run` from the start button. Type 'cmd' and press enter. you are in the windows command prompt. Now install the module 'flask'
...
Consider writing code for a box. A box has 3 properties-height, width, breadth. We can use variable like `box_height`, `box_width`, `box_breadth` to represent them. But doing this makes 3 seperate variables that seem to have no relation. Instead, we could create a *class* that holds these variables. Classes are neat way to organize code.
```{#numCode .python .numberLines}
class Box(object):
...
We are creating a class called `Box` that contains variables `height`, `weight` and `breadth`. So, think of class as a container to holding varaibles.  line 1 to 15 is the class defination. 
A Class can also has functions. We defined the method `volume()` on line 13,14 within the class.

**Object creation and method calling**

Line 17 creates a box object with `height` as 2, `weight` as 3 and `breadth` as 4. At this line, the special function `__init__()` is called with 2,3,4 as parameters. I know, there is no call to `__init__()`. Here is the thing with python: If a class is defined with name `XYZ` then we create an object by calling `XYZ()`. However, inside the class there is no such method. Instead the class `XYZ`'s `__init()__` is called. This is special method called **constructor**. Another object is created at line .. Notice the print stmt 'creating object' executed each time an object is created.

Only one class can exist but several objects can exist. On line 18 and line 21, are creating 2 objects. These 2 are different from each other. Each object have a set of `height, weight, breadth` and have no connection from another. 

Notice that the contructor defination on line 2 has 4 parameters while the method call on line 17 we had only 3 parameters. This is because all class methods have a default first parameter as `self` which doesn't have be passed when the method is called. You can notice this with the method `volume()` as well. The method defination on line 13 has one parameter but on line 18, while calling there is none.

`self` is special variable that refers to the object itself.  line 3 to line 5, we define the 3 instance variables. Since variables are not declared in python, we need to assign them values in the constructor itself. Also notice the use of the `self` variable to refer instance variables. If we don't use `self`, python would think the variable is a global variable. Our Box class has 3 varaibles and there are assigned inside the constructor.
...
Notice that the South_Indian_Family had no methods or variables defined. But, we will able to access `southy.members`. This is because South_Indian_Family derived the varialbe `members` from its base class.
...
Exceptions are errors that the programmer can't stop from happening. Like when trying to read from a file, the file is not available. Or when a program tries to find something on the internet but there is no internet. Since exceptions can't be prevented, there should atleast be handled. Which means, We need to provide some safety code. As usual, examples will clearify further.
...
print 'enter numerator:'
```
All the code that can throw an exception is written in the `try` block. A `try` block is followed by an `except` block, where code that handles the exception situation is written. Line 4 throws an exception. All the rest of the code in `try` block is not executed and program control is passed to the `except` block. The `except` block catches the exception. The `except` block is executed.

As we can in the output, the program safely continues taking input even in the case of a Divide by zero error. 

Let's break this again. This time we will give invalid input.
