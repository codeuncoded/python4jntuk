...
This is a *tiny, tiny* book. It was meant to be tiny. It's 50 pages. Without the lab exercises it's 34 pages. Why? They say, a picture is worth a thousand words. Similarly, an example is worth a page of explanation. Most often, dry paragraphs are replaced with examples. 
...
```
We can see that we were able to element-wise add from 3 lists without the need to declare a new function.
...
print dict(collections.Counter(name))
```
**Explanation:** `collections.Counter` is built-in dict class that counts the frequecies of elements. It also has the method `most_common()` that returns the most common element.
...
C programs need a semicolon on each statement. Python needs none. And a text file might contain a semicolon buy not as much as a C file.
...
```
**Explanation:** `with` is keyword that lets us create context managers. Context managers are a clearer way to write code. Notice that in this solution, we didn't close the file. That is done by the context manager. Context managers are not in the syllabus so don't worry if you don't understand them. Just remember that they are neat way to organize code.
...
Mode is the element that has the most common. So, we count the frequencies of all elements and find the element with the highest frequency. 
...
Greatest Common Diviser(gcd) of two numbers a,b lies between 1 to a or b which is the maximum. So, we create a list of all the numbers that are from 1 to `max(a, b)` and also divide a,b prefectly. Since gcd is the maximum of all these numbers, we pop the last element. 

Least Common Multiple(lcm) of tow numbers a,b lies between max(a,b) and a*b. So, we create a list of all such numbers and which are also perfectly divided by a,b. Since lcd is the min of all these numbers, we pop() the first element. 
